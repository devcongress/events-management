...
Decision: Keep email preflight inside the existing Hono application. A shared module performs bounded syntax normalization, high-confidence provider-typo suggestions, a maintained disposable-domain lookup, and ordered Cloudflare then Google Public DNS-over-HTTPS JSON lookups with the RFC-defined A/AAAA fallback and null-MX rejection. Each resolver attempt has its own short timeout; the secondary resolver is contacted only when the primary returns an HTTP/runtime failure or an inconclusive DNS status. EMS-hosted forms call a narrowly rate-limited preflight endpoint to render a real **Checking email** then **Submitting** button sequence. Every final public submission endpoint repeats the same server-side policy so clients cannot bypass it. Definite invalid results stop only the new submission; all-resolver timeouts, failures, and other inconclusive results fail open. Cache domain-level results only, never complete addresses. Structured resolver-failure logs include correlation, resolver, record type, failure class/status, and duration but exclude the submitted address and domain. Do not add a database column, migration, historical backfill, check-in restriction, email challenge, or claim that domain deliverability proves mailbox ownership.

Trade-offs: This catches common errors and low-effort disposable addresses without a click-through gate, but it cannot detect a valid address entered by someone else or prove that an individual mailbox exists. DNS adds one bounded external read on an uncached domain and, only when the primary is unavailable, exposes that domain query to the secondary provider. Community-maintained disposable lists can have false positives or lag new providers. Rechecking final endpoints adds server work but preserves the security boundary; short-lived domain caching and fail-open resolver handling keep the registration path available.
