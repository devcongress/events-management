...
- **Event Archive + requests**
  ...
  - Approval and rejection are final. Both desktop and phone review flows require an explicit cannot-be-undone confirmation before the API transition; the phone confirmation is a safe-area-aware bottom drawer and the desktop confirmation remains an app modal. Selecting a proposal prepares a reproducible, event-bound 128-bit `go.devcongress.org` capability whose raw value is never stored; the unique stored hash resolves the compact code, while already-issued longer codes remain valid. The organizer can preview all ready selected-speaker emails together when at least two are waiting, or preview and send exactly one speaker's email from that proposal row; selection alone never emails the speaker. Preview loading is keyed to the active row, and a newer preview aborts the stale request rather than changing every row's state.
