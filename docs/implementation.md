...
- **December 2026 annual-conference workspace and volunteer intake**
  ...
  - Monthly meetup finance: monthly events expose a private `/organizer-console/events/:eventId/finance` workspace with no budget form. It records a GHS actual-expense ledger per meetup, summarizes actual, paid, unpaid, and cancelled amounts, and keeps monthly expenses separate from the Annual Conference budget model. Owners and Organizers can view and manage every monthly finance ledger.
  - Monthly meetup finance APIs: `GET /api/events/:eventId/finance`, `POST /api/events/:eventId/finance/categories`, `POST /api/events/:eventId/finance/expenses`, and `PATCH /api/events/:eventId/finance/expenses/:expenseId`; reads, category creation, and expense mutations require an Owner or Organizer session, Volunteers are rejected, categories are shared across monthly meetups but independent from Annual Conference finance, all are monthly-event-only and validated at the server boundary, and mutations are audited.
