...
- **Monthly system design**
  - Active Vue pages: `src/views/admin/AdminSystemDesignView.vue`, `src/views/SystemDesignPresenterView.vue`, `src/views/PlayCodeView.vue`, `src/views/EventView.vue`, `src/views/ArchiveEventView.vue`
  ...
  - Learning room: `src/components/SystemDesignLearningRoomPanel.vue` generates and reviews exactly five source-based questions on the saved-artifact page. `POST /api/quiz/sessions/[sessionId]/presentation` prepares a fresh anonymous run after completion without expiring the reusable content; `/present/system-design/:sessionId` opens separately outside the admin shell.
- **Attendance analysis**
...
- **Public event consumer preview:** authenticated organizers open `/organizer-console/website-preview/events` from Event Management to inspect the published collection returned by `/api/public/meetups`. Every card opens `/organizer-console/website-preview/events/:slug`, whose website-shaped detail is backed by `/api/public/meetups/:slug`. The preview shows the exact endpoint and links to its JSON, remains available under the phone route policy, and does not read private `/api/events` records or expose unpublished drafts.
...
- `src/views/EventsView.vue` is the authenticated consumer-preview collection. It mirrors the DevCongress website meetup listing shape, reads only `/api/public/meetups` through the shared TanStack Query cache, and routes every published card into the matching preview detail rather than bypassing it for registration or archive links.
- `src/views/EventView.vue` is the authenticated consumer-preview detail. It consumes `/api/public/meetups/:slug` to render the meetup cover, schedule resources, speakers, status action, past-event photos, and recordings. Third-party media is embedded only for exact HTTPS YouTube or Vimeo player hosts inside a restricted iframe and matching Content Security Policy allowlist; other validated URLs remain explicit external links.
...
- `src/views/admin/AdminMobileOrganizerView.vue` is the deliberately limited phone-only event list. It shows at most three live/upcoming/recent event cards with safe registration or source links, and **Check in guests** navigates to `src/views/admin/AdminMobileCheckInView.vue` rather than expanding inside the card. That standalone door-mode screen removes the global organizer chrome, keeps a visible **Back to events** action, identifies the selected event, and owns name/email search, first-letter filtering, progress, and full-width check-in actions. The shared `RegistrationAlphabetFilter.vue` exposes only initials present in the guest list through 44px touch targets, while `src/lib/registration-checkin.ts` combines that selection with name/email search and stable alphabetical ordering. Existing events without a native campaign show a bounded historical-registration explanation instead of search controls or synthetic guests. Cancellation/removal remain in the full console. Full setup, editing, attendance/feedback operations, access, reporting, audit, and bulk table work stays tablet/laptop-only; the canonical mobile routes prevent hidden desktop route chunks from loading behind them.
...
- `src/views/admin/AdminEventsView.vue` reads the organizer event list through the shared TanStack query layer, shows focusable detail popovers for the compact lifecycle legend, creates classified native events with draft registration campaigns, auto-generates an editable website slug from the event name, uploads a selected/compressed cover after the event record exists, and invalidates the event-list plus overview queries after creating a new event.
...
- **Non-obvious logic:** `GET /api/quiz/state` is read-only. Attendee requests do not receive live answer distribution before reveal; the standalone authenticated presenter opts into that room pulse with `presenter=true`.
