...
| File | Role |
|---|---|
| `src/main.ts` | Vue app bootstrap — waits for initial router readiness, then selects the public or organizer root before mounting Vue Router, TanStack Query, global CSS, and delegated button press feedback |
| `src/App.vue` | Protected organizer app shell and top navigation, including the organizer-access boot surface |
| `src/PublicApp.vue` | Public-only app shell with no organizer header, navigation, or session bootstrap |
| `lib/app-boot.ts` | Shared route-aware first-paint markup and styles injected into the Vite build shell and Hono development fallback |
...
| `lib/email/previews.ts` | Owner email-preview catalog that maps every active scenario to the existing production renderer using safe sample fixtures |
...
- **Event Archive + requests**
  ...
  - The organizer proposal table shows every status by default and uses the shared app-styled All/Pending/Approved/Rejected dropdown above the table. The review drawer shows the complete proposal before selection and the complete archive record after completion/publication. Archive status actions are intentionally available only from that content-first drawer.
  - Approval and rejection are final. Both desktop and phone review flows require an explicit cannot-be-undone confirmation before the API transition; the phone confirmation is a safe-area-aware bottom drawer and the desktop confirmation remains an app modal. Selecting a proposal prepares a reproducible, event-bound 128-bit `go.devcongress.org` capability whose raw value is never stored; the unique stored hash resolves the compact code, while already-issued longer codes remain valid. Selection marks the delivery pending before recording the final decision, then dispatches the acceptance email immediately. A scheduled worker retries pending or failed selected-speaker deliveries with the same event/link idempotency key; accepted deliveries are never selected again. The table exposes Sending, Sent, and Retrying states rather than making delivery an extra organizer task.
  - Owners can create one submitted, owner-only test proposal per event to test the actual decision and delivery lifecycle. It is hidden from other organizers. The separate table-level **Send test to me** endpoint accepts only a client request UUID, derives the recipient from the authenticated Owner session, renders fixed synthetic speaker content for the current event, and records no speaker-link delivery state.
...
- **December 2026 annual-conference workspace and volunteer intake**
  - Active Vue pages: `src/views/admin/AdminAnnualConferenceView.vue`, `src/views/admin/AdminAnnualConferenceWorkPlanView.vue`, `src/views/admin/AdminAnnualConferenceFinanceView.vue`, `src/views/VolunteerIntakeView.vue`, `src/views/admin/AdminVolunteerView.vue`, `src/views/admin/AdminVolunteerDisplayView.vue`
  - Organizer workspace: `/organizer-console/annual-conference/:year`, with an edition-scoped Work plan; Volunteers remains a 2026-specific compatibility workflow. Legacy Timeline links redirect to Work plan while retaining phase and task context. The workspace header uses one edition selector beside the Annual Conference label, places module navigation on its own row, and exposes future-edition creation as a secondary action. Desktop and tablet Kanban columns show newly created cards and cards that changed status or phase first, using server-managed board-entry metadata; existing cards retain their established order and phone layouts continue to use persisted `sort_order`.
  ...
  - Work-plan phase contract: 2026 seeds Phase 1 for 1–31 August and Phase 2 for 1 September–19 December. Confirmed kickoff work is assigned to Phase 1 while the remaining tasks stay unclassified. Direct visits default to the current or next phase; an explicit **Entire conference** selection is stored as `phase=all`, so optimistic status updates retain the selected scope. Organizers can switch to another phase, No phase, or the entire conference. Future editions can use any number of phases; deleting one returns its tasks to **No phase**.
  ...
  - Finance workspace: /organizer-console/annual-conference/:year/finance is visible only when the response carries finance.view; it reports GHS planned budget, committed, paid, remaining, unpaid committed, outstanding expected income, received income, and category variance. Owners can open a right-side drawer to add budget lines or explicit expense/income records; the ledger's manual-income **Manage** flow records partial receipts, expectation changes, and unpaid cancellations while retaining their history. Selected Organizers are read-only finance viewers; Owners grant that view access in People & Access.
  - Finance APIs: GET /api/annual-conference/:year/finance, POST /api/annual-conference/:year/finance/budgets, POST /api/annual-conference/:year/finance/entries, PATCH `/finance/entries/:entryId/expected`, POST `/finance/entries/:entryId/receipts`, and POST `/finance/entries/:entryId/cancel`; finance reads require finance.view, mutations require an Owner session, and each mutation is audited.
  - Finance storage: relational Supabase annual_conference_finance_budgets, annual_conference_finance_entries, annual_conference_finance_income_amendments, and annual_conference_finance_income_receipts with GHS constraints and service-role-only grants; production receipt/amendment mutations use transactional Postgres functions and lib/mock-db/annual-conference-finance.ts remains the local JSON fallback. Entry source markers reserve source-linked rows for future ticket and sponsor modules.
  - Task contract: exactly `Not started`, `In progress`, `Blocked`, and `Done`; no reminders; receipts, reimbursements, approval policy, reconciliation, and multi-currency finance remain later slices.
  - Organizer compatibility redirects: `/organizer-console/volunteers` and `/organizer-console/volunteer-display`
  - Public form: `/volunteer` is the canonical standalone route without organizer navigation or app chrome; `/volunteer/december-mega-meetup` redirects to it for distributed-link compatibility. Its public copy is an evergreen DevCongress volunteer invitation, while submissions continue to feed the legacy 2026 campaign and organizer directory. Name and email are required; X and DevCongress Slack names are optional. The compact form retains touch-friendly controls, shared email preflight, and mandatory production Turnstile verification.
  - Volunteer display: `/organizer-console/annual-conference/:year/volunteers/display` is a protected, TV-safe DevCongress volunteer invitation with a high-contrast QR focal point and a responsive QR-first stack for smaller screens. It prefers the one active `volunteer_intake` short link and falls back to the canonical `/volunteer` URL when short-link storage is unavailable.
  ...
  - Follow-up campaign: `server/routes/volunteer-follow-up.ts` owns the 2026 queue, Owner launch/control, review-capability projections, private final-answer form, and scheduled drain. The read route only fetches persisted recipients and overlaps that read with Owner-only delivery health; it never reconciles applicants or sends mail. `lib/supabase/volunteer-follow-up.ts` reconciles the existing intake document into relational recipients during launch and scheduled recovery, and enrolls new or duplicate applications with an idempotent per-applicant insert. Failed intake enrollment is logged without applicant data and repaired by the 15-minute scheduler, which reconciles draft, paused, and running campaigns independently of email configuration and quota. Migration `20260922200000_volunteer_follow_up_campaign.sql` serializes daily claims and provides an expiring database drain lease. `src/components/VolunteerFollowUpPanel.vue` is shared by desktop and phone Volunteers. The standalone `src/views/VolunteerFollowUpView.vue` uses a centered single-column layout, two identically styled question cards with borderless semantic fieldsets, a full-width motivation field with a live word counter, and compact native attendance radios with the app-pink selected state; live applicant submission retains its private token and Turnstile verification. The public `/volunteer/follow-up/test` route is a separate sandboxed test form: it verifies a test-specific Turnstile action, applies a separate public rate limit, and always discards answers without database writes, email, or queue work. Owners can open it in a new tab or copy its persistent `go.devcongress.org` short link; the legacy protected preview path redirects there. The initial skeleton follows the progress, role-specific setup/timeline, and inbox layout; background refresh failures leave the last loaded data visible. Directory, Reviews, and Campaign mount on their first visit and remain mounted while inactive so tab switches preserve loaded data and controls. The response window is 14 days after the Owner-configurable application cutoff; the initial cutoff is 30 September 2026, Accra time. The cutoff becomes immutable at launch because invitation copy promises it. Daily sends are bounded by 54, a 35-email reserve, and complete Resend usage freshly read at each drain; the queue stops if an accepted send omits quota headers. Launch is explicit, never automatic on deploy. The invitation drawer contains the production email and its plain-text alternative only.
  - Reviews and Campaign are separate connected tabs on desktop and phone. `GET /api/annual-conference/2026/volunteer-follow-up/reviews` and review PATCH require `volunteers.review_applications` and return an allowlisted reviewer DTO; invitations are reduced to sent/not-sent. The campaign GET/settings/control/preview routes are Owner-only. Campaign payloads whitelist settings and operational delivery fields and never return the database drain lease token. Session identity scopes client query keys and Owner cache is removed on role downgrade.
  - Selection decisions and outcome email delivery: the reviewer PATCH atomically saves review status, note, and expected-version decision without sending. The Owner previews a ten-minute eligible/excluded snapshot with immutable personalized payloads and must explicitly confirm; the transaction rejects cohort drift and deduplicates repeated confirms. Outcome delivery has separate pause/resume controls and shares the invitation lease and daily claim budget. See `docs/features/volunteer-outcomes.md` for the durable retry, webhook, and ambiguity contract.
...
- **Attendance analysis**
  ...
  - The hub pairs the monthly ledger with one compact attendance-pattern panel: approved RSVP/check-in bars for the latest recorded events, an approved came/missed pie, and a small venue-planning footer. Completed native monthly campaigns derive this data from registrations/check-ins in bounded batch reads; future events remain unrecorded, and older events retain the Luma CSV fallback.
...
- `supabase/migrations/20260728000000_native_event_registrations.sql` adds relational campaigns, attendee records, check-ins, retryable confirmation deliveries, and the atomic free-registration function. The organizer check-in endpoint has a paired `DELETE` mutation that removes only the selected check-in row for a correction while preserving the guest registration.
...
- `src/main.ts` starts initial router resolution before mounting Vue, preserving the server/Vite-injected route-specific boot screen through a hard-refresh route load. It mounts `src/PublicApp.vue` for deliberate public exceptions and lazy-loads `src/App.vue` only for organizer/protected routes, then installs Vue Router, the shared TanStack Query plugin, and the delegated pointer-only button press-feedback system from `src/button-press-feedback.ts`.
- `lib/app-boot.ts` keeps the organizer first-paint copy for protected and unknown paths while rendering registration, CFP, feedback, speaker-intake, volunteer, and System Design learning-room skeleton variants for public paths. `server/index.ts`, `server/app.ts`, `vite.config.ts`, and `public/_worker.js` select the same variant before Vue mounts so local Bun, Vite, Hono, and Cloudflare Pages do not regress to the organizer loader.
...
- `src/views/admin/AdminMobileOrganizerView.vue` is the lightweight phone Home and does not load event or conference operating data. The mobile menu exposes **Home**, **Events**, and **Conference** as separate destinations. `src/views/admin/AdminMobileEventsView.vue` owns the complete event list and opens `AdminMobileEventView.vue` for focused Overview, live Guests, and presentation Submissions. The Submissions tab shows the stable reviewable proposal total, while pending, selected, and not-selected filters use the shared `src/lib/speaker-submission-counts.ts` projection so their pink counters move with each decision; the desktop presentation review filters consume the same projection. **Check-in mode** opens the standalone `AdminMobileCheckInView.vue` door-mode screen with name/email search, first-letter filtering, progress, full-width check-in actions, and confirmed-guest undo controls. **Email guests** opens `AdminMobileEventBlastsView.vue`, which reuses the existing confirmed-audience, 100-recipient, capacity-reserve, scheduling, rendered-preview, persisted-provider-draft, audited-send, and prepared-failure retry contract; shared starter copy lives in `src/lib/event-blast-workspace.ts`. `src/views/admin/AdminMobileAnnualConferenceView.vue` is a standalone role-specific workspace: organizers receive Overview, filtered Work Plan, Volunteers, edition controls, and full task create/edit flows; volunteers receive only assignment-scoped Overview and My Tasks with status-only controls. It reuses the shared workspace controller, API capabilities, task form, and task drawer without loading desktop route chunks. The shared `RegistrationAlphabetFilter.vue` exposes only initials present in the guest list through 44px touch targets, while `src/lib/registration-checkin.ts` combines that selection with name/email search and stable alphabetical ordering. Existing events without a native campaign show a bounded historical-registration explanation instead of search controls or synthetic guests. Cancellation/removal remain in the full event console; canonical mobile routes prevent hidden desktop route chunks from loading behind them.
- `src/views/admin/AdminAttendanceOverviewView.vue` renders the monthly attendance ledger and a compact two-chart pattern panel from `/api/attendance/monthly`. The charts compare approved RSVPs with check-ins across recent recorded events and summarize selected-year approved RSVP outcomes; native monthly campaigns supply registrations/check-ins automatically, while older campaign-less meetups remain eligible for Luma CSV import. Its people panel defaults to regular attendees with at least two actual check-ins and restores the registered, came, rate, and last-seen summary. A separate **Never came** view lists every person with at least two approved RSVPs and zero check-ins, excluding pending and declined registrations. The people table uses a capped, keyboard-focusable scroll area with a sticky column header so long follow-up lists do not lengthen the entire page.
...
- `src/lib/event-form.ts` centralizes Zod validation for organizer event creation so the create form and `/api/events` server endpoint share the same required-field, slug, date, and URL rules. Its date helpers synchronize the default event range and require an optional end time to be strictly after the start.
