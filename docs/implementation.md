...
- **December 2026 annual-conference workspace and volunteer intake**
  ...
  - Finance workspace: /organizer-console/annual-conference/:year/finance is visible only when the response carries finance.view; it reports GHS planned budget, committed, paid, remaining, unpaid committed, income, and category variance. Owners can open a right-side drawer to add budget lines or explicit expense/income records; selected Organizers are read-only finance viewers.
  - Finance APIs: GET /api/annual-conference/:year/finance, POST /api/annual-conference/:year/finance/budgets, and POST /api/annual-conference/:year/finance/entries; finance reads require finance.view, mutations require an Owner session, and record creation is audited.
  - Finance storage: relational Supabase annual_conference_finance_budgets and annual_conference_finance_entries with GHS constraints and service-role-only grants; lib/mock-db/annual-conference-finance.ts is the local JSON fallback.
