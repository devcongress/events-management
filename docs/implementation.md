...
- `src/views/admin/AdminLoginView.vue` presents the selected responsive Programme Cover sign-in as the sole organizer login design. It exposes Google OAuth only, verifies that server-side Supabase organizer auth is configured before enabling the action, presents transient OAuth callback failures as app toasts while keeping configuration and connectivity failures inline, and owns its mobile scroll container inside the standalone login shell.
