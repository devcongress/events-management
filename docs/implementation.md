...
- `src/views/admin/AdminMobileOrganizerView.vue` is the phone-only organizer surface. It shows current/next event cards, public/registration/source links, and feedback QR access when feedback is open, while full setup, editing, reporting, audit, and bulk table work stays tablet/laptop-only.
