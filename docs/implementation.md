...
- `src/components/ui/AppDatePicker.vue` supplies the organizer forms with one themed date/date-time control. Its body-level popover uses the visual viewport to open above or below the trigger, clamps horizontally on narrow screens, and scrolls internally when neither side can fit the full calendar. Inline errors are exposed through the trigger's accessible error state and description.
