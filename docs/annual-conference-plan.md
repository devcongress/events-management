...
- the Overview replaces the Volunteer Intake callout with a compact workload view for Owners and Organizers: outstanding work is grouped once per accountable owner, matching legacy aliases against the active member directory where possible; selecting a person opens the entire Work plan filtered to that person even when their work sits outside the current phase, while direct Work plan visits still default to the current phase; volunteer sessions do not receive the team-wide view;
