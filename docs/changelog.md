...
## 2026-08-08 — Local conference-speaker queue preview

- Added an opt-in, browser-only sample queue for the Annual Conference Speakers workspace with submitted, selected, and not-selected proposals; it never creates or changes shared speaker records.
- Included enough submitted examples to exercise the shared pagination control and an explicit preview banner with an exit action.
- Replaced the large proposal cards and status tiles with one compact directory table: submitted, selected, and not-selected totals live in the panel header and double as the only filters.
