...
- `primary_action` is an additive, event-specific CTA override. Consumers should prefer it over their status-derived registration or recording action. Every event whose name contains `Project Night` receives `{ "kind": "slack_profile", "label": "Message @aberkowitz", "url": "slack://user?team=T0A0T7A5Q&id=U3LB1TNLS" }`, including externally classified and past events; other events receive `null`. This deliberate exception reflects the named event owner's approved contact instruction. The Slack URI opens the member profile in the native client, where the visitor can choose **Message**.
