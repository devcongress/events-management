...
Every currently open monthly CFP, event registration, and Annual Conference CFP has one canonical flyer link. Opening the registry backfills any existing open form that predates this capability; copying an eligible public link in its own workspace creates or reuses the same code thereafter. The public URL stays short and opaque while the form remains open.
...
1. Apply `20260809120000_short_links.sql`, then `20260809193000_short_link_canonical_destinations.sql`.
