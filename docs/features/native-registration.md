...
7. On event day, use **Guests** to combine status, first-letter, and name/email filters, then select **Check in**. The first-letter rail remains hidden until the first guest exists, so an empty list has no inert **All** control. Ordinary registered guests do not carry a redundant “Confirmed” badge; the list calls out only waitlisted, cancelled, checked-in, or post-event no-show states. Tablet and desktop guest rows scroll inside a bounded region so the controls remain visible; phones retain one natural page scroll. Tablets use the event’s full Registration tab; on phones, **Check in guests** opens a dedicated event screen with its own back action, event identity, progress, filters, and guest actions.
