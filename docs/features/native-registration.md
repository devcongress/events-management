...
After an official monthly meetup ends, the Monthly Attendance overview uses its native campaign registrations and check-ins directly. It batches those reads across the ledger and never waits for or stores a duplicate end-of-event snapshot; future meetups remain in their pre-event state. Older monthly meetups without a native campaign retain the Luma CSV import workflow until that legacy data path is retired.
...
7. On event day, use **Show QR code** to display the registration form on a shared screen or phone. Guests scan with their camera, complete the existing name/email form, and appear in the same live guest list. The display also offers a direct **Open form on this phone** fallback and the short registration URL. Use **Guests** to combine status, first-letter, and name/email filters, then select **Check in**; outside the event date, the same recognizable **Check in** action remains visible but disabled. If the wrong person is checked in, select **Undo check-in** and confirm; this removes only the attendance record and keeps the registration active. Organizers can select **Remove** to take a registration out of the active guest list without hard-deleting its historical record; removing a confirmed guest may promote the next waitlisted person. The first-letter rail remains hidden until the first guest exists, so an empty list has no inert **All** control. Ordinary registered guests do not carry a redundant “Confirmed” badge; the list calls out only waitlisted, cancelled, checked-in, or post-event no-show states. Tablet and desktop guest rows scroll inside a bounded region so the controls remain visible; phones retain one natural page scroll. Tablets use the event’s full Registration tab. On phones, each event opens a dedicated **Overview / Guests / Submissions** workspace: Guests exposes the live searchable registration list and safe check-in/undo actions, while **Check-in mode** provides the more focused event-day screen. **Show registration QR** and **Email guests** remain available from Overview; Email guests preserves the desktop Blasts audience, capacity, scheduling, preview, history, and retry safeguards in a single-column phone flow.
...
9. Use **Blasts** for one custom event-update email to confirmed guests only. The composer opens with the editable **Reminder** draft by default, while Event update and Venue change remain available starters, then opens one rendered email preview whose primary action sends or schedules the message. The renderer treats the editable message as the complete body, so a greeting entered by the organizer appears exactly once. Blast emails deliberately use the same branded event-details layout as registration receipts—including the DevCongress wordmark, one compact **When / Where** itinerary card, a map pin, side-by-side calendar actions, and explicit light/dark email colours—so guests receive one coherent event communication system. The first release stops at 100 recipients and preserves the message as **Needs email capacity** when the dedicated Resend Broadcast key or provider capacity is unavailable. Before an immediate send, EMS subtracts queued transactional work and a protected daily reserve from Resend's last observed quota; an oversized blast is saved without delivery. **Blasts** shows the usable total, allocation for blasts, transactional reserve, and audience as labeled information rather than actions. Select **Manage today’s allocation** to edit the two coupled values; increasing one reduces the other within the current usable total. Saving immediately refetches campaign and capacity state. Recent activity is a paginated table; select any row for a right-side detail drawer with its delivery timing, audience preparation progress, provider references, message, and any concrete failure reason plus retry action. After confirmation, EMS captures the confirmed audience once and imports ten recipients per queue message. Three durable Resend segment slots are shared across all event blasts, so a fourth waits until a slot's broadcast reaches a confirmed terminal status and its audience membership has been removed in bounded queue work. New sends and retries are dispatched through the queue; provider status is checked before retrying a send whose response may have been lost. A stable slot name lets a retry find the segment if provider creation succeeded but the Worker could not persist its ID. The history row reports audience preparation progress; no broadcast is sent until the immutable recipient snapshot is ready. A failed audience preparation returns a safe, concrete explanation and can resume after the cause is corrected. Waitlisted and cancelled guests are never added, and the app never sends a partial blast. Before enabling the pool, remove legacy one-off blast segments from Resend once so the three fixed slots can be provisioned within the current plan limit; after that initial cleanup, EMS reuses the same three segments.
...
| Table | Purpose |
|---|---|
| `event_registration_campaigns` | One private registration introduction, capacity, window, automatic overflow, lifecycle record, and optional event-specific blast reserve per event; legacy auto-confirm/waitlist columns remain internal policy fields |
...
| `event_blasts` | Organizer-owned subject/body, immutable confirmed-recipient snapshot, batch preparation progress/error, preparation/scheduled/sent/capacity state, and provider reconciliation IDs for custom event updates |
| `event_blast_segment_slots` | Three service-role-only persistent Resend segments with atomic blast ownership; slots remain reserved until terminal provider status and empty membership are confirmed |

All six tables have RLS enabled and no anonymous table policies. Public writes go through the validated Hono endpoint using the server-only Supabase client. In production, the endpoint requires action/hostname-bound Turnstile verification and applies atomic cross-Worker limits by client and normalized email. The browser's rate-limited email preflight is repeated by the final registration endpoint; definite invalid domains are rejected, while DNS timeouts and other inconclusive resolver results continue to registration. DNS queries use the Workers-native `node:dns` API enabled by the existing `nodejs_compat` configuration; structured resolver diagnostics omit both the attendee address and its domain. This is deliverability preflight, not mailbox-ownership verification, and it introduces no registration or check-in state. Each campaign permits one active registration per normalized email. The `register_for_event` database function locks the campaign row while allocating capacity, preventing concurrent submissions from over-confirming the event. The service-role-only `cancel_registration_and_promote` function takes the same campaign lock, cancels the selected guest, promotes only the oldest waitlisted guest when a confirmed place opens, and queues the promotion notice in the same transaction. Draft campaigns return the same not-found contract as an unknown registration link so unpublished event details are not exposed; scheduled and closed campaigns reject public submissions on the server.

## Email Delivery

Confirmation email uses the existing server-only `RESEND_API_KEY` integration with the code-owned attendee-facing `DevCongress Events <events@updates.devcongress.org>` sender. `REGISTRATION_EMAIL_REPLY_TO` remains required deployment configuration, and registration never falls back to the distinct speaker-program identity. Blasts use a separate `RESEND_BROADCASTS_API_KEY`; Resend owns per-recipient unsubscribe links and scheduled delivery. The app creates an event-specific Resend Segment at send time so an event blast cannot accidentally address all DevCongress contacts. The Queue adds guests sequentially in ten-recipient batches, respecting Resend `429` retry delays; it persists an incomplete audience for an explicit resume and never creates the broadcast until every snapshot recipient is ready.
...
| `src/views/admin/AdminMobileOrganizerView.vue` | Compact phone orientation and links into focused operations |
| `src/views/admin/AdminMobileEventsView.vue` | Phone event list with a primary path into each event workspace |
| `src/views/admin/AdminMobileEventView.vue` | Dedicated phone Overview, live guest list, and speaker/product-demo proposal review with safe single-record actions |
| `src/views/admin/AdminMobileEventBlastsView.vue` | Focused phone blast composer, rendered preview, delivery capacity, history, and prepared-send retry |
...
| `src/lib/event-blast-workspace.ts` | Shared Reminder, Event update, and Venue change starter copy used by desktop and phone |
...
| `lib/event-blast-preparation.ts` | Queue message contract and bounded ten-recipient audience preparation batch |
...
| `supabase/migrations/20260828211500_event_blast_resumable_preparation.sql` | Immutable audience snapshot and durable queue preparation progress/error |
| `supabase/migrations/20260925100000_event_blast_segment_pool.sql` | Three atomic reusable Resend segment slots and guarded terminal/release RPCs |
