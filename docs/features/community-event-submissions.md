...
`POST /api/public/event-submissions/with-cover` accepts the same validated fields as multipart form data plus one optional public cover image. It is explicitly unauthenticated, like the coverless intake route, and then applies the same launch gate, Turnstile, rate limits, storage checks, and validation. It exists separately so the normal JSON route remains capped at 64KB; the cover route is capped just above 5MB and accepts only AVIF, JPEG, PNG, or WebP files after extension, MIME, and file-signature validation. The website renders a client-only event preview before either route is submitted. Draft recovery deliberately excludes the image file.
