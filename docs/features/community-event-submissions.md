...
An approved organizer receives one private, time-bounded management link. It is a signed bearer capability backed by a service-role-only link record, so it can be revoked without exposing a submission lookup. New links keep the capability in the URL fragment, which browsers do not send in HTTP request paths, and the amendment client presents it to same-origin API routes in the `Authorization` header. Legacy path-bearing links remain accepted so already-issued emails do not break; application logs redact those legacy paths, while retention or removal of old provider URL logs remains a Cloudflare operational task outside this repository. The capability expires at the canonical event end time, including an approved change to that time; expired, revoked, withdrawn, and ended events cannot be mutated. Organizers can retrieve the existing active URL from the approved-submission drawer with **Copy management link** when a sender says they did not receive the approval email. That action creates no email or quota usage and records only the copy event and expiry in the audit ledger, never the bearer URL. The organizer can only propose schedule, location, online, registration, an optional replacement cover image, and an optional reviewer note; title, description, format, and organizer identity remain fixed. The management form is seeded from the current approved community event after each accepted amendment, while the original submission remains unchanged as historical review evidence. Its schedule picker uses the event’s declared time zone, rather than the browser’s time zone, and labels that time zone beside the fields; unchanged times retain their original instant through repeated daylight-saving hours. A changed time that occurs twice during a clock change is rejected with a clear request for an unambiguous time. Before submission, **Review changes** compares the form with the current approved event and lists only changed schedule, location, online link, registration link, and cover fields. A venue-only edit shows the location change without adding an unchanged schedule. Equivalent timestamp strings for the same instant are treated as unchanged, while changing only the declared time zone remains a schedule change. Editing any field after opening the review clears the review state, and submitting requires a current review with at least one event-detail change. The cover is staged with the amendment and does not replace the published image unless the organizer approves the entire change request. Saving drafts and submitting a change request send no email. A successfully submitted request posts a best-effort alert to the existing private submission Slack channel with the requested schedule and location plus a direct link to the Updates review drawer; a missing or unavailable webhook never rolls back the submitted request. Only one draft or submitted request can exist at a time. The Organizer compares the submitted request with the latest canonical community event in the existing submission drawer; if that baseline is unavailable, the drawer says so and prevents approval. Approval transactionally updates the canonical event, clears the reviewed monitor findings, rebaselines monitoring on the new canonical details, and schedules the next source-page comparison at the event's normal cadence. Approval does not fetch the source page or send a monitoring Slack alert immediately, and it never republishes the event announcement to the public Slack channel. Decline leaves the public listing and monitoring baseline untouched and lets the organizer prepare another request. Both decisions use the existing durable email outbox. Rejected initial proposals never receive a management link and must be resubmitted as a new proposal.

An organizer can select the IANA timezone an amendment should use. Switching the selector preserves the current instants so the organizer can deliberately edit the displayed clock times, then confirm the requested schedule in both its selected timezone and Ghana time. The accepted timezone flows to the canonical public event; the original submission timezone remains historical evidence.
...
Eligible published events (current or future only) also keep a durable Events-channel Slack announcement record. EMS checks the exact `devcongress.org/events/...` page before reserving or sending a Slack post, so a static-site 404 or unreachable page can never produce a broken public link. The organizer details explain that the post waits for the next website refresh. A Worker scheduled drain checks eligible announcements that were not claimed because the website was unavailable every 15 minutes, so a website build that completes at 6:00, 7:00, or 8:00 is picked up automatically on the next successful check. Provider failures remain explicit **Retry Slack** actions, while the Organizer can still use **Send to Slack** manually. A successful announcement is terminal and the action disappears, preventing accidental duplicate posts.

When `SLACK_EVENTS_BOT_TOKEN` and `SLACK_EVENTS_CHANNEL_ID` are configured, EMS sends through `chat.postMessage`, stores only the returned channel ID and message timestamp, and prefers that path over the compatibility webhook. Later edits to the public event name, schedule, format, location, cover, or slug rebuild the original card with `chat.update`; approved community-event amendments use the same synchronization path. The event edit remains saved if Slack is unavailable, while the announcement record retains a bounded update error for organizer visibility. The message shows the complete start-to-end time range when both values are available. Webhook-posted legacy announcements remain readable but cannot become editable without a one-time message reference because incoming webhooks do not return one. Apply `20260810230000_event_slack_announcements.sql` and `20260909113000_editable_event_slack_announcements.sql`, configure `SLACK_EVENTS_RETRY_SECRET`, install a Slack bot with `chat:write`, and invite it to `#events` before enabling editable delivery.

The event overview presents the public website and Slack events channel as independent publication states. Website readiness is checked for every published listing even when Slack delivery is not configured; a missing static page is shown as waiting, while connectivity or provider errors remain explicit failures with a manual recheck. Slack shows its own not-sent, sending, sent, update-failed, and delivery-failed outcomes, together with the most recent successful send or update. For bot-posted messages, the read model resolves the stored channel and timestamp through Slack's permalink API so an Organizer can reopen the exact announcement. Permalink lookup is read-only and best-effort; it does not affect event loading, sending, retry, or idempotency. Legacy webhook messages have no provider reference and therefore remain visible without an open-announcement action.
