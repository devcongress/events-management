...
On a phone, use the menu to move between **Home**, **Events**, and **Conference**. Home provides direct, thumb-sized entry points to the Events and Conference workspaces. Events contains the event list and a dedicated three-part workspace for each event: **Overview** for context and quick links, **Guests** for the live registration list and check-in actions, and **Submissions** for full speaker and product-demo proposals plus select/not-select decisions. From Overview, **Email guests** opens a focused Blasts workspace with the confirmed audience, capacity protection, Reminder/Event update/Venue change starters, optional scheduling, mandatory rendered preview, delivery history, and safe failed-send retry. Advanced event setup, bulk audience selection, guest cancellation, and destructive administration remain desktop-only. Conference opens the dedicated Annual Conference workspace. Organizers receive mobile versions of **Overview**, the searchable and filterable **Work plan**, and the 2026 **Volunteers** area. Organizers with the corresponding permissions can switch or create editions and create and fully edit tasks. Task and edition forms use full-screen phone flows rather than compressed desktop tables. Volunteers receive **Overview** and **My tasks** by default, showing only assigned work, hiding internal organizer notes, and allowing status updates only. Their current authorized edition is shown without edition-management controls; when no work is assigned, desktop and mobile show one clear no-action-required state instead of an empty progress dashboard. Explicit responsibilities add the matching conference-wide data and mobile tabs.
...
Open **Work plan** to see overall status and the filtered task list. For a selected phase, its header compares completed work with elapsed phase time and shows the remaining calendar time; an owner alert appears only when tasks still need assignment. On desktop, Owners, Organizers, and assigned-only Volunteers see that list as a status board inside the same workspace surface. A member can drag a card between status columns only when they are its accountable owner or collaborator; the platform owner and the edition planning owner can move any card. The card moves immediately while its update saves in the background and remains draggable: a rapid second move queues its newest status, while other cards remain available. If the final save fails, only that task returns to its last confirmed column and the error explains why. The compact owner group uses the same Navii character for each person as their task cards: select a visible character to filter directly to that owner, or select the `+N` badge to open every owner by full name. A shared link that carries a status, workstream, or attention filter shows it as a removable chip beside those controls. Volunteers do not see an inert **Clear** control; it appears only while they have active filters. Open a task for its details or editor. On phones, volunteers retain their compact task list. These filters and the open task are part of the URL, so a shared link, a move between desktop and phone, or browser Back returns to the same useful context; an explicit Entire conference selection is retained while a task status saves. Choose a phase and target date in the task drawer; dates remain optional, and an assigned target date cannot exceed the phase end. The platform owner and edition planning owner can add and edit every task; other organizers can edit tasks where they are accountable or collaborating. A read-only task explains the applicable permission instead of presenting an empty action area.
...
For later meetups, use CFP to open or close the public proposal form and Proposals to select or reject submissions. Approval and rejection are final decisions, so review the full proposal before choosing either action. Selecting a proposal creates its private `go.devcongress.org` completion link and immediately queues its acceptance email. The table shows **Sending**, **Sent**, or **Retrying** so delivery is never invisible; failed delivery is retried by the scheduled worker with the same provider idempotency key, so a retry cannot create a duplicate message. Owners can create an owner-only **test proposal** to exercise this real approval flow without exposing it to other organizers, or use **Send test to me** for a synthetic rendering sample that changes no proposal or delivery state. When the participant submits the private form, the result enters the same Event Archive model as the July manual path; it is not a second archive or a different record type.
