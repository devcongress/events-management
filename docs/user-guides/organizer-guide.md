...
For later meetups, use CFP to open or close the public proposal form and Proposals to select or reject submissions. Approval and rejection are final decisions, so review the full proposal before choosing either action. Selecting a proposal automatically prepares its private `go.devcongress.org` completion link. In the Approved view, choose **Preview email(s)** to inspect the exact system-generated message for each selected speaker; no email is sent until you confirm **Send N emails** in the preview. Already-sent or completed speakers are skipped. When the participant submits the private form, the result enters the same Event Archive model as the July manual path; it is not a second archive or a different record type.
