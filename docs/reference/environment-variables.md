...
| `SPEAKER_INTAKE_LINK_TOKEN_SECRET` | Required when selecting monthly-event speakers | No | Server-only HMAC secret of at least 32 random bytes. It derives the high-entropy `go.devcongress.org` capability stored only as a hash with each selected-speaker link, allowing EMS to reproduce the short URL after a reload. |
