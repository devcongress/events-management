...
| `SLACK_EVENTS_RETRY_SECRET` | Required for scheduled Slack retries | No | Secret of at least 32 random bytes used only by the Worker scheduled trigger to authorize the internal retry drain. Set with `wrangler secret put SLACK_EVENTS_RETRY_SECRET`; do not expose it to the frontend. |
