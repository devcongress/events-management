...
- Keep `SHORT_LINK_RESOLVER_TOKEN` and `SPEAKER_INTAKE_LINK_TOKEN_SECRET` out of browser code and out of `wrangler.toml`. The resolver token is shared by EMS and the short-link Pages project; the speaker-intake secret belongs only to EMS. `SHORT_LINK_PUBLIC_ORIGIN`, `EMS_RESOLVER_ORIGIN`, and `PUBLIC_APP_ORIGIN` are safe normal variables in their respective runtimes.
