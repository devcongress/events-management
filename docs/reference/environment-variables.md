...
| `RESEND_BROADCASTS_API_KEY` | Required to send or schedule event blasts | No | Separate server-only **Full access** Resend key. EMS must create Contacts, Segments, and Broadcasts before sending; Resend's sending-only keys cannot do that. The app saves a friendly capacity state when it is missing or the provider rejects the send for plan/quota reasons. |
