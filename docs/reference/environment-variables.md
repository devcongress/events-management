...
- Generate shared application secrets with `openssl rand -base64 32`. The app rejects the management-link, reply-routing, short-link resolver, and scheduled-retry secrets when their trimmed UTF-8 value is shorter than 32 bytes. Do not rotate an existing strong secret merely to satisfy this check: rotating management/reply secrets invalidates existing signed links, and rotating the short-link resolver token requires an atomic update in both Cloudflare projects.
