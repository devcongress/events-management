...
| `R2_ACCOUNT_ID` | Required for R2 backup upload | No | Cloudflare account ID used to build the private R2 S3 endpoint |
| `R2_BACKUP_BUCKET` | Required for R2 backup upload | No | Private R2 bucket that stores only encrypted Supabase backup archives |
| `R2_ACCESS_KEY_ID` | Required for R2 backup upload | No | Bucket-scoped R2 S3 access key ID with Object Read & Write access |
| `R2_SECRET_ACCESS_KEY` | Required for R2 backup upload | No | Bucket-scoped R2 S3 secret access key; keep only in the local environment or GitHub Actions secrets |
| `R2_BACKUP_PREFIX` | No | No | Safe object-key prefix for encrypted archives; defaults to `events-management/supabase` |
...
| `SHORT_LINK_RESOLVER_TOKEN` | Required when short links are enabled | No | Shared secret of at least 32 random bytes used only between the isolated `go.devcongress.org` Pages Function and the EMS internal short-link resolver. Store the same value as a secret in EMS and the short-link Pages project. |
...
| `RESEND_BROADCASTS_API_KEY` | Required to send or schedule event blasts | No | Separate server-only **Full access** Resend key. EMS manages three persistent blast segments, Contacts, and Broadcasts; Resend's sending-only keys cannot do that. Remove legacy one-off blast segments once before enabling the pool if they use the plan's three-segment limit. |
...
| `RESEND_WEBHOOK_SECRET` | Required for outbound delivery tracking | No | Server-only Resend/Svix signing secret for `/api/webhooks/resend`; separate from `RESEND_INBOUND_WEBHOOK_SECRET`. |
| `VOLUNTEER_FOLLOW_UP_TOKEN_SECRET` | Required before volunteer follow-up can send | No | Server-only random HMAC secret of at least 32 bytes for recipient-bound private form links. Keep stable across deployments and never expose to the browser. |
...
| `SPEAKER_INTAKE_LINK_TOKEN_SECRET` | Required when selecting monthly or Annual Conference speakers | No | Server-only HMAC secret of at least 32 random bytes. It derives reproducible high-entropy speaker workspace capabilities while the database stores only their hashes. |
...
| `EVENT_SUBMISSION_REPLY_TOKEN_SECRET` | Required for EMS-routed submission replies | No | Server-only HMAC secret of at least 32 random bytes used to create and verify submission-specific Reply-To addresses. Never expose or commit it. |
| `EVENT_SUBMISSION_MANAGEMENT_TOKEN_SECRET` | Required for approved community-listing amendment links | No | Server-only HMAC secret of at least 32 random bytes that signs the revocable no-login management links issued after a community event is approved. Never expose or commit it. |
...
| `SLACK_EVENT_SUBMISSION_WEBHOOK_URL` | Optional for submission alerts | No | Server-only Slack incoming webhook URL for the private submission/review channel. New submissions, submitted amendment requests, and email replies are posted there; EMS remains the source of truth if Slack is unavailable. |
...
| `SLACK_EVENTS_BOT_TOKEN` | Required for editable event announcements | No | Server-only Slack bot token with `chat:write`. The bot must belong to the same app that posts the event messages and be invited to the configured channel. Never expose or log it. When both bot settings are ready, EMS prefers the Web API over the legacy incoming webhook. |
| `SLACK_EVENTS_CHANNEL_ID` | Required for editable event announcements | No | Stable Slack channel ID for `#events` (for example, `C…`). EMS stores the channel and returned message timestamp after `chat.postMessage`, then uses them for bounded `chat.update` calls after public event-detail changes. |
| `SLACK_EVENTS_RETRY_SECRET` | Required for scheduled event jobs | No | Existing secret of at least 32 random bytes used only by the Worker scheduled trigger to authorize the internal Slack retry and registration-page monitoring drains. Set with `wrangler secret put SLACK_EVENTS_RETRY_SECRET`; do not expose it to the frontend. No additional monitor secret is required. |
...
- Keep production Turnstile hostnames limited to `devcongress.org,www.devcongress.org`. Use Cloudflare's dummy Turnstile keys for local tests instead of authorizing `localhost` or `127.0.0.1` on the production widget.
...
- Keep `SHORT_LINK_RESOLVER_TOKEN`, `SPEAKER_INTAKE_LINK_TOKEN_SECRET`, and `SLACK_EVENTS_BOT_TOKEN` out of browser code and out of `wrangler.toml`. The resolver token is shared by EMS and the short-link Pages project; the speaker-intake secret and Slack bot token belong only to EMS. `SHORT_LINK_PUBLIC_ORIGIN`, `EMS_RESOLVER_ORIGIN`, `PUBLIC_APP_ORIGIN`, and `SLACK_EVENTS_CHANNEL_ID` are safe normal variables in their respective runtimes.
...
- Owners can use `/api/health/data-sources` to compare local and deployed persistence and inspect the non-sensitive `missing` / `weak` / `ready` status of protected shared secrets. The endpoint never returns secret values. The server falls back to `local-json` when local/dev runs omit `APP_DATA_SOURCE`, but the committed example selects `supabase` because organizer auth requires it. Matching `supabase.project_ref` values mean Supabase-backed domains are using the same project. Domains reported as `supabase-json` share the `app_json_documents` bridge table; domains reported as `local-json` still read from each runtime's local data files.
...
- Keep the R2 backup bucket private with no `r2.dev` or custom public domain. Scope its S3 credentials to that bucket only, and store the GitHub copies as `R2_BACKUP_ACCESS_KEY_ID` and `R2_BACKUP_SECRET_ACCESS_KEY`; the workflow maps them to the script's runtime names.
- Generate application secrets with `openssl rand -base64 32`. The app rejects the management-link, reply-routing, selected-speaker-link, short-link resolver, and scheduled-retry secrets when their trimmed UTF-8 value is shorter than 32 bytes. Rotating `SPEAKER_INTAKE_LINK_TOKEN_SECRET` invalidates uncompleted selected-speaker short links, so keep it stable; rotating the short-link resolver token requires an atomic update in both Cloudflare projects.
