...
describe("Annual Conference assignee work-plan routing", () => {
  it("passes the selected assignee from Overview and applies it across the entire conference", () => {
    ...
    expect(workPlanSource).toMatch(
      /watch\(\s*\[\(\)\s*=>\s*route\.fullPath, tasks, phases, organizerMembers, assignedAccess\]/u,
    );
  ...
  it("retains the current-phase initialization for direct work-plan visits", () => {
    ...
    expect(workPlanSource).toContain(
      "defaultAnnualConferencePhaseScope(phases.value, today.value)",
    );
  });

  it("stores desktop filters, including an explicit Entire conference scope, and open tasks in the route", () => {
    expect(workPlanSource).toContain(
      "router.push({ path: route.path, query: workPlanContextQuery(patch) })",
    );
    expect(workPlanSource).toContain(
  ...
  it("uses route history for mobile sections and task drawers", () => {
    ...
    expect(mobileConferenceSource).toContain(
      "pushMobileConferenceContext({ task: taskId })",
    );
    expect(mobileConferenceSource).toMatch(
      /watch\(\s*\[\(\)\s*=>\s*route\.fullPath, tasks, phases, assignedAccess\]/u,
    );
  });
  ...
  it("keeps volunteers in a focused board without a redundant assigned-only owner control", () => {
    ...
    expect(mobileConferenceSource).toMatch(
      /ownerFilter\.value = assignedAccess\.value\s*\?\s*["']all["']/u,
    );
  });
  ...
  it("keeps phase and owner controls in one labelled controls section", () => {
    ...
    expect(workPlanSource).toContain(
      "!ownerAvatarPreviews.value.some((preview) => preview.key === owner.key)",
    );
    expect(workPlanSource).toContain("selectedOwnerAvatarSeed(owner)");
    expect(workPlanSource).toContain(
      "More owner filters. Current filter: ${selectedOwnerLabel}",
    );
    expect(workPlanSource).toContain(
  ...
  it("applies attention filters consistently in desktop and mobile Work plans", () => {
    expect(workPlanSource).toMatch(
      /matchesAnnualConferenceTaskAttention\s*\(\s*task\s*,\s*attentionFilter\.value\s*,\s*today\.value\s*,?\s*\)/u,
    );
    expect(mobileConferenceSource).toMatch(
      /matchesAnnualConferenceTaskAttention\s*\(\s*task\s*,\s*attentionFilter\.value\s*,\s*today\.value\s*,?\s*\)/u,
    );
  ...
  it("uses the desktop Work plan board for organizers and volunteers", () => {
    ...
    expect(workPlanSource).toMatch(
      /return role === ["']owner["']\s*\|\|\s*role === ["']organizer["'];/u,
    );
    expect(workPlanSource).toContain(
      "function canMoveTask(task: AnnualConferenceTask): boolean",
    );
    expect(workPlanSource).toContain(
      "permissions.value?.can_update_all_task_status",
    );
    expect(workPlanSource).toContain(
  ...
  it("keeps the filtered board inside the shared Work plan workspace surface", () => {
    ...
    expect(workPlanSource).toContain(
      "annual-task-workspace__controls flex flex-wrap items-center justify-between gap-4 border-b border-dc-ink",
    );
    ...
    expect(workPlanSource).toContain(
      'class="annual-task-ledger rounded-b-lg border-x-2 border-b-2 border-dc-ink bg-dc-paper"',
    );
    expect(workPlanSource).toContain(
      "top: var(--annual-conference-nav-height);",
    );
    expect(taskBoardSource).toContain("top: var(--task-board-sticky-offset);");
    expect(taskBoardSource).toContain(
  ...
  it("queues board status moves per task without locking the whole board", () => {
    ...
    expect(workPlanSource).toContain(
      "updateMutation.isPending.value || selectedTaskStatusSaving",
    );
    expect(workPlanSource).not.toContain(
      "updateMutation.isPending.value || task.status === status",
    );
    expect(workspaceSource).toContain(
    ...
    expect(workspaceSource).toContain(
      "previousStatus = variables.previousStatus ?? cachedStatus",
    );
  ...
  it("keeps board cards compact and opens their details from the card surface", () => {
    ...
    expect(taskBoardSource).toContain(
      "No blocked tasks in this view. Drop a task here.",
    );
