...
export const ORGANIZER_PHONE_EVENT_BLASTS_ROUTE_NAME =
  "admin-mobile-event-blasts";
export const ORGANIZER_PHONE_CHECK_IN_ROUTE_NAME = "admin-mobile-check-in";
export const ORGANIZER_PHONE_ANNUAL_CONFERENCE_ROUTE_NAME =
  "admin-mobile-annual-conference";
export const ORGANIZER_VOLUNTEER_FOLLOW_UP_PREVIEW_ROUTE_NAME =
  "admin-volunteer-follow-up-form-preview";

export type OrganizerMobileEventSection = "overview" | "guests" | "submissions";
export type OrganizerMobileConferenceSection =
  | "overview"
  | "tasks"
  | "volunteers";
...
const PHONE_ALLOWED_ADMIN_ROUTE_NAMES = new Set([
  ...
  "admin-organizers",
  ORGANIZER_VOLUNTEER_FOLLOW_UP_PREVIEW_ROUTE_NAME,
]);
...
const DESKTOP_CONFERENCE_ROUTE_SECTIONS = new Map<
  string,
...
const DESKTOP_EVENT_ROUTE_SECTIONS = new Map<
  string,
...
const CONFERENCE_STATUSES = new Set([
  "all",
  "not_started",
  "in_progress",
  "blocked",
...
const CONFERENCE_ATTENTION_FILTERS = new Set([
  "overdue",
  "due_soon",
  "needs_planning",
]);
...
function redirectTarget(
  path: string,
  query: Record<string, string> = {},
): OrganizerViewportRedirectTarget {
  return {
    path,
...
export function organizerMobileEventSection(
  query: OrganizerRouteQuery | undefined,
): OrganizerMobileEventSection {
  const section = queryString(query, "section");

  return section === "guests" || section === "submissions"
    ? section
    : "overview";
}

export function organizerMobileConferenceSection(
  query: OrganizerRouteQuery | undefined,
): OrganizerMobileConferenceSection {
  const section = queryString(query, "section");
...
export function organizerConferenceContextQuery(
  ...
  if (attention && CONFERENCE_ATTENTION_FILTERS.has(attention))
    context.attention = attention;
  if (workstream && CONFERENCE_WORKSTREAMS.has(workstream))
...
export function isOrganizerPhoneRouteName(
  routeName: string | symbol | null | undefined,
): boolean {
  return (
    routeName === ORGANIZER_PHONE_ROUTE_NAME ||
...
export function matchesOrganizerPhoneViewport(
  target: Pick<Window, "matchMedia"> | undefined = typeof window === "undefined"
    ? undefined
    : window,
): boolean {
...
export function organizerPostAuthLanding(
  ...
  const isConferenceOverview = new RegExp(
    `^${adminPath("annual-conference/")}\\d{4}/?$`,
  ).test(target);
...
export function organizerViewportRedirect({
  ...
  if (isPhone) {
    ...
    if (DESKTOP_CONFERENCE_ROUTE_NAMES.has(normalizedRouteName)) {
      const section =
        DESKTOP_CONFERENCE_ROUTE_SECTIONS.get(normalizedRouteName) ??
        "overview";
    ...
    if (eventId) {
      const section =
        DESKTOP_EVENT_ROUTE_SECTIONS.get(normalizedRouteName) ?? "overview";
  ...
  if (normalizedRouteName === ORGANIZER_PHONE_EVENT_ROUTE_NAME && eventId) {
    ...
    if (section === "guests")
      return redirectTarget(
        adminPath(`events/${encodeURIComponent(eventId)}/registrations`),
      );
    if (section === "submissions")
      return redirectTarget(
        adminPath(`events/${encodeURIComponent(eventId)}/talks/proposals`),
      );
  ...
  if (
    normalizedRouteName === ORGANIZER_PHONE_EVENT_BLASTS_ROUTE_NAME &&
    eventId
  ) {
    return redirectTarget(
      adminPath(`events/${encodeURIComponent(eventId)}/registrations`),
  ...
  if (normalizedRouteName === ORGANIZER_PHONE_ANNUAL_CONFERENCE_ROUTE_NAME) {
    ...
    if (section === "tasks") {
      return redirectTarget(
        annualConferencePath("work-plan", conferenceYear ?? undefined),
        targetQuery,
      );
    }
    if (section === "volunteers")
      return redirectTarget(
        annualConferencePath("volunteers", conferenceYear ?? undefined),
      );

    return redirectTarget(
      annualConferencePath("", conferenceYear ?? undefined),
    );
  }
