...
describe("mobile organizer entry points", () => {
  it("turns the organizer home into permission-aware next actions", () => {
    ...
    expect(mobileHomeSource).toContain(
      ':to="ORGANIZER_PHONE_EVENTS_ROUTE_PATH"',
    );
  ...
  it("shows edition management only after an organizer role is confirmed", () => {
    ...
    expect(mobileConferenceSource).toContain(
      'v-if="canManageEditions" class="conference-mobile__edition-bar"',
    );
    expect(conferenceNavSource).toContain(
      'v-if="canManageEditions" class="min-w-44 max-w-56',
    );
    expect(conferenceNavSource).toContain(
    ...
    expect(conferenceNavSource).not.toContain(
      "gap-4 border-t border-dc-border pt-4",
    );
  });

  it("keeps the volunteer conference overview focused and removes organizer navigation", () => {
    expect(mobileConferenceSource).toMatch(
      /v-if="!isVolunteer"\s+:to="ORGANIZER_PHONE_ROUTE_PATH"/u,
    );
    expect(mobileConferenceSource).toContain(
      "'conference-mobile__header--volunteer': isVolunteer",
    );
    expect(mobileConferenceSource).toContain(
      "assignedAccess && summary.total === 0",
    );
    ...
    expect(mobileConferenceSource).toMatch(
      /position: sticky;\s*top: calc\(4\.15rem \+ env\(safe-area-inset-top\)\)/u,
    );
  });

  it("gives desktop volunteers the same focused zero-assignment state", () => {
    expect(desktopConferenceSource).toContain(
      "'conference-brief--volunteer': assignedAccess",
    );
    expect(desktopConferenceSource).toContain(
      "assignedAccess && summary.total === 0",
    );
  ...
  it("keeps mobile volunteer actions and directory filters together while scrolling", () => {
    expect(mobileConferenceSource).toContain(
      'class="volunteer-directory-sticky"',
    );
    expect(mobileConferenceSource).toMatch(
      /\.volunteer-directory-sticky\s*\{\s*position: sticky;/u,
    );
    expect(mobileConferenceSource).not.toContain(
      ".volunteer-directory-tools { position: sticky;",
    );
  });
  ...
  it("pins the desktop volunteer directory controls beneath the conference navigation", () => {
    ...
    expect(desktopVolunteerSource).toContain(
    ...
    expect(desktopVolunteerSource).toContain(
      "min-w-[60rem] table-fixed border-collapse text-left md:min-w-0",
    );
    expect(desktopVolunteerSource).toContain(
      "top: var(--annual-conference-nav-height);",
    );
    expect(desktopVolunteerSource).toMatch(
      /top:\s*calc\(\s*var\(--annual-conference-nav-height\)\s*\+\s*var\(--volunteer-directory-toolbar-height\)\s*\);/u,
    );
    expect(desktopVolunteerSource).not.toContain(
      'class="editorial-panel volunteer-directory-panel overflow-hidden"',
    );
    expect(desktopVolunteerSource).toContain(
