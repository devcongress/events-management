...
describe("organizer viewport policy", () => {
  ...
  it("keeps the owner-only volunteer form preview available on phone viewports", () => {
    expect(
      organizerViewportRedirect({
        authenticated: true,
        isAdminRoute: true,
        isPhone: true,
        routeName: ORGANIZER_VOLUNTEER_FOLLOW_UP_PREVIEW_ROUTE_NAME,
      }),
    ).toBeNull();
  });
  ...
  it("preserves intentional post-login destinations and volunteer routing", () => {
    const taskTarget =
      "/organizer-console/mobile/annual-conference/2026?section=tasks&task=task-1";
    const eventTarget = "/organizer-console/mobile/events/event-1/check-in";

    expect(organizerPostAuthLanding(taskTarget, "organizer", true)).toBe(
      taskTarget,
    );
    expect(organizerPostAuthLanding(eventTarget, "owner", true)).toBe(
      eventTarget,
    );
    expect(
      organizerPostAuthLanding(
    ...
    expect(organizerPostAuthLanding(eventTarget, "volunteer", true)).toBe(
      "/organizer-console/annual-conference/2026",
    );
  });
  ...
  it("maps event subpages to mobile sections without losing the event", () => {
    ...
    expect(
      ...
      path: "/organizer-console/events/event%2Fone/talks/proposals",
      replace: true,
    });
  });
  ...
  it("redirects a legacy Timeline context to the Work plan", () => {
    ...
    expect(organizerMobileConferenceSection({ section: "timeline" })).toBe(
      "tasks",
    );
  });
  ...
  it("degrades unsupported context to safe defaults", () => {
    ...
    expect(organizerMobileEventSection({ section: "unknown" })).toBe(
      "overview",
    );
  });
  ...
  it("returns tablets and desktops from the limited route to the full console", () => {
    ...
    expect(
      ...
      path: "/organizer-console/events/event-one/registrations",
      replace: true,
    });
    expect(
      organizerViewportRedirect({
  ...
  it("builds encoded dedicated phone routes", () => {
    expect(ORGANIZER_PHONE_EVENTS_ROUTE_PATH).toBe(
      "/organizer-console/mobile/events",
    );
