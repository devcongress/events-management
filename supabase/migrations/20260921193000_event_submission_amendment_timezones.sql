-- Amendment times are wall-clock values in an organizer-selected IANA zone.
-- Preserve older drafts by backfilling their canonical event zone first.

alter table public.event_submission_amendments
  add column if not exists timezone text;

update public.event_submission_amendments amendment
set timezone = coalesce(event.timezone, submission.timezone, 'UTC')
from public.event_submissions submission
left join public.community_events event on event.id = submission.approved_event_id
where amendment.submission_id = submission.id
  and (amendment.timezone is null or btrim(amendment.timezone) = '');
...
begin
  ...
  select * into current_event from public.community_events where id = submission.approved_event_id for update;
  if not found then raise exception 'event_submission_not_approved'; end if;

  if p_approve then
    if current_event.ends_at <= now() then raise exception 'event_submission_management_window_closed'; end if;
    if amendment.ends_at <= now() then raise exception 'event_submission_amendment_ends_in_past'; end if;

    next_location_name := case when amendment.location_type = 'online' then 'Online' else coalesce(nullif(trim(amendment.venue_name), ''), nullif(trim(current_event.location_name), ''), 'Venue to be announced') end;
    next_location_label := case when amendment.location_type = 'online' then 'Online' else coalesce(nullif(trim(amendment.venue_address), ''), nullif(trim(amendment.venue_name), ''), nullif(trim(current_event.location_label), ''), next_location_name) end;
    ...
    update public.event_submission_management_links set expires_at = amendment.ends_at where submission_id = submission.id and revoked_at is null;
  end if;

  update public.event_submission_amendments set
  ...
  values (submission.id, amendment.id, (case when p_approve then 'amendment_approved' else 'amendment_rejected' end)::public.event_submission_email_kind, 'event-submission-' || submission.id::text || '-amendment-' || amendment.id::text || case when p_approve then '-approved' else '-rejected' end)
