...
create unique index short_links_one_active_global_destination_idx
on public.short_links (destination)
where status = 'active' and destination = 'volunteer_intake';
...
begin
  if input_destination in ('monthly_cfp', 'event_registration') then
...
begin
  ...
  elsif current_link.conference_edition_id is not null then
    perform pg_advisory_xact_lock(hashtext(current_link.destination::text || ':' || current_link.conference_edition_id::text));
  else
    perform pg_advisory_xact_lock(hashtext(current_link.destination::text));
  end if;
