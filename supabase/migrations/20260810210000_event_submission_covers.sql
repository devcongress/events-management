-- Allow public community-event submissions to carry an optional verified cover.
-- Existing migration files are immutable, so approval is overridden forward-only.
...
alter table public.event_submissions
  drop constraint if exists event_submissions_cover_url_not_blank;

alter table public.event_submissions
  add constraint event_submissions_cover_url_not_blank
...
begin
  ...
  insert into public.community_events (
    ...
    submission.starts_at, submission.ends_at, 'upcoming',
      coalesce(nullif(trim(submission.cover_url), ''), '/images/quarterly-april-meetup-2.jpeg'),
