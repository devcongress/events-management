...
add constraint short_links_destination_target check (
  ...
  or (destination in ('volunteer_intake', 'volunteer_follow_up_test') and event_id is null and conference_edition_id is null)
);

create unique index short_links_one_active_global_test_destination_idx
on public.short_links (destination)
where status = 'active' and destination = 'volunteer_follow_up_test';
...
begin
  ...
  elsif input_destination in ('volunteer_intake', 'volunteer_follow_up_test') then
    if input_event_id is not null or input_conference_edition_id is not null then
      raise exception 'Invalid global short-link destination.' using errcode = '22023';
    end if;
  else
