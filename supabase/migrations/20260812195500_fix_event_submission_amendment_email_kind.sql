-- PostgreSQL resolves the amendment decision CASE expression as text. Cast it
-- to the outbox enum so both approval and rejection can commit atomically.
...
begin
  ...
  insert into public.event_submission_email_deliveries (
    ...
    amendment.id,
    (case
      when p_approve then 'amendment_approved'
      else 'amendment_rejected'
    end)::public.event_submission_email_kind,
