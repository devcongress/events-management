...
function toPublicMeetup(event: Event, eventTalks: Talk[], origin: string): PublicMeetup {
  ...
  return {
    ...
    primary_action: projectNightPrimaryAction(event.name),
...
const eventBlastPreparationMessageSchema = z.object({
...
app.post('/api/quiz/sessions/:sessionId/release', async (c) => {
  ...
  const updated = await updateQuizSession(session.id, {
    ...
    question_started_at: new Date(Date.now() + (SYSTEM_DESIGN_ANSWER_START_DELAY_SECONDS * 1000)).toISOString(),
